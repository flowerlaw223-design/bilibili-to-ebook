...
## 5.5 四种视频、四种打法（2026-09-14 实测，四本真实产出）

同一套流程跑过四类视频，差别极大。**动手前先判断类型，再套对应打法。**

| 类型 | 例子 | 切章 | 配图 | 图注 | 打法 |
|---|---|---|---|---|---|
| **PPT 讲课** | AI Agent 编年史（46 分钟） | ✅ 自动 | 60 张 | ✅ | 首选，全自动 |
| **滚动文档** | AI Agent 面试复盘（36 分钟） | ⚠️ 人工点边界 | 90 张 | ✅ | 半自动 |
| **录屏演示** | AI 编程全流程（25 分钟） | ⚠️ 人工点边界 | 48 张 | ⚠️ 少数漏网 | 半自动，图注需抽查 |
| **纯口播** | 面试经验分享（27 分钟） | ⚠️ 人工点边界 | **不配图** | — | 只做正文，别硬配图 |

### 三条由此得出的硬规则

**规则一：口播型视频不配图。**
滤掉界面帧后内容画面不足 5 个 → 整本不配图。
实测：27 分钟纯口播，真正内容画面只有 4 个（片头卡、一张 PPT、片尾卡、更新预告），
其余全是共享屏幕时的浏览器/Word 界面。硬配出来只是和正文重复的装饰图。
**必须在书的「关于本书」里写明原因**，否则读者以为漏了图。

**规则二：界面外壳不是内容，选帧阶段就要滤掉。**
坑：选帧偏爱"文字最多"的帧，而口播视频里文字最多的恰恰是浏览器书签栏 / Word 菜单栏——
算法会**主动挑中垃圾**。
判据（两个都要，缺一不可）：
- 硬痕迹：含 URL、书签栏品牌名 ≥3（百度/京东/淘宝…）、Office 功能区专属词（另存为/兼容性模式…）、中文不足 4 字；
- **像不像人话**：菜单词 ≥3 **且** 虚词密度 < 3%（菜单栏里没有"的了是在和就也"）。

> 反向踩坑警告：**别用"工具/设计/格式/搜索/设置"这类泛词判界面**——
> 讲技术方案的 PPT 里这些词遍地都是，会把整本判成菜单栏（实测配图 49 张 → 2 张）。

**规则三：图像去重阈值必须随视频自适应。**
各类视频的视觉基线差得离谱：PPT 全场一套模板（相邻帧图像相似度中位 **96%**）、
录屏 93%、滚动文档 57%、纯口播 100%。
**固定阈值 0.66 会把 PPT 那本从 49 张打到 2 张。**
正确做法：`effective = max(阈值, 本视频相邻帧相似度中位数)` —— 闸门永远不低于视频自己的基线。

### 逐帧去重的正确尺子

- **不要用字符集合（1-gram）**：滚动文档滚两行，句子全变了，但两屏中文共享大量常用字，
  相似度仍然很高，拦不住"几乎一样的两张图"。
- **用句子级 4-gram，并且跟最近 4 张比**（只跟上一张比会漏掉"隔一张的近重复"——
  实测抓出过一张与前面第 2 张重复 62% 的图）。
- 实测效果：相邻配图重复度中位 1~2%，同时每一帧的内容仍能被某张入选图覆盖到（不是靠丢内容换的）。

### 验收：必须让人看，不能只看指标

模型读不了图。**"这两张图像不像"是语义判断，任何文本/像素指标都可能对不上人的眼睛**——
本项目连续三次指标与人眼判断相反。
所以每本书都生成**配图总览**（`tools/contact_sheet.py`：网格缩略图 + 时间点 + 与前图重复度），
**请人扫一眼指认具体是哪两张**，再据此标定阈值。这是唯一可靠的验收方式。
