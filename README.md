...
# 另外需要: ffmpeg、pandoc（pandoc 缺失时会自动回退到内置 EPUB 构建器）

# 2. 自检：看看本机工具齐不齐
export PYTHONPATH=src          # Windows: set PYTHONPATH=src
python -m bbook doctor

# 3.（可选）检查 cookie 是否还能登录 —— 决定能否取官方字幕
python -m bbook cookies /path/to/cookies.txt

# 4. 一键跑通
python -m bbook run "https://www.bilibili.com/video/BVxxxxxxxxxx/" \
       --cookies /path/to/cookies.txt \
       --terms terms/ai-coding.json \
       --workdir work/BVxxxxxxxxxx
```

分阶段执行（推荐首次使用，便于逐段核对）：

```bash
python -m bbook probe  "<url>"  --workdir work/BVxxx     # Phase 1 元数据
python -m bbook audio  "<url>"  --workdir work/BVxxx     # Phase 2 音频
python -m bbook asr    work/BVxxx --model small          # Phase 2 本地转写
python -m bbook clean  work/BVxxx --terms terms/ai-coding.json   # Phase 3 清洗 + 术语归正
#   → 人工/LLM 填写 work/BVxxx/chapters.json（章节划分，模板见 examples/）
python -m bbook build  work/BVxxx                        # Phase 7-8 构建 + 审计
```
